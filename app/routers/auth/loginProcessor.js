...
module.exports = async function login(db, ctx, dbField, dbValue, authFields, authValues, name0, email0, lat0, lon0) {
  const [user] = await db.query(`SELECT id, name, email, isAdmin, lat, lon, settings FROM user WHERE ${dbField} = ?`, [dbValue]);
